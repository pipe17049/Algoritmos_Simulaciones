...
for timep in range (totaltiempo+q):     #Ciclo for para el tiempo con la variable timep hasta que llege al acumulador + 1

    for x in range(len(proceso)):

        if timep == delay[cont2]:                   #Pregunta si el tiempo transcurrido total es igual al delay del proceso que sigue
    ...
    if cont>0 and cont < q and cont>=proceso[ejecucion[0]]:             #Compara si la variable cont esta entre 0 y el quantum y si es igual al tiempo de proceso
        ...
        elif len(cola)==0 and len(ejecucion)==1 and proceso[ejecucion[0]]==0:
            break
